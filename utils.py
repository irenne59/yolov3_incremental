...
def compute_distillation_loss(old_p, p, targets, model, giou_loss=False):  # predictions, targets, model
    print('=============== Start computing distillation loss ===============')
    fileHandler = open('lwf-3.txt', 'a')
    ft = torch.cuda.FloatTensor if p[0].is_cuda else torch.Tensor
    # if old_p[0].is_cuda:
    #     print('pred is cuda')
    # else:
    #     print('pred is torch.tensor')
    lxy, lwh, lcls, lconf = ft([0]), ft([0]), ft([0]), ft([0])
    # -- lxy, lwh, lcls, lconf zero values tensor

    txy, twh, tcls, tbox, indices, anchor_vec = build_targets(model, targets)
    # -- txy, twh = target (gt) bounding box location
    # -- tcls = target (gt) class ID 
    # -- tbox = target bounding box
    # -- indices = collection of array index in target tensor for b = bounding box, a = anchor box, gj = grount truth y, gi = ground truth x
    # -- anchor_vec = anchor box vectors
    ...
    fileHandler.write('Batch size: ' + str(bs) + '\n')
    k = bs / 64  # loss gain
    print('Loss gain: ' + str(k))
    fileHandler.write('Loss gain: ' + str(k) + '\n')

    # -- if p as in prev train simulation is tensor array of size 3 x 4 x 3 x 15 x 15 x 85 [0, 1, 2, 3, 4, 5]
    for i, pi0 in enumerate(p):  # layer i predictions, i
        # -- pi0: [1, 2, 3, 4, 5]
        b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx, indices = ground truth generated by current model
        ...
        if len(b):  # number of targets
            ...
            # -- pi: [5]
            print('pi size, should be class num + 4 + 1 = ' + str(len(pi)) + ' x ' + str(len(pi[0])))
            fileHandler.write('pi size, should be class num + 4 + 1 = ' + str(len(pi)) + ' x ' + str(len(pi[0])) + '\n')
            print('pi, predictions closest to anchors [6th index - n + 4 + 1]: ' + str(pi)) # 3 times display as per a (3 classification layers)
            fileHandler.write('pi, predictions closest to anchors [6th index - n + 4 + 1]: ' + str(pi) + '\n')
            ...
            # print('tclsm: ' + str(tclsm)) #-- zero tensor (mxn)
            print('tclsm size: ' + str(len(tclsm)) + ' x ' + str(len(tclsm[0])))
            tclsm[range(len(b)), tcls[i]] = 1.0
            print('Length of b: ' + str(len(b)))
            print('Range of len b ' + str(range(len(b))))
            print('tcls[i]: ' + str(tcls[i]))
...
def build_targets(model, targets):
    ...
    for i in model.yolo_layers:
        ...
        if nt:
            iou = [wh_iou(x, gwh) for x in layer.anchor_vec] # iou between default anchor boxes and ground truth (width, height)
            ...
            if reject:
                j = iou > iou_thres
                t, a, gwh = targets[j], a[j], gwh[j] # t = ground truth image?, a = default-anchor box, gwh = ground truth width height

        # Indices
        b, c = t[:, :2].long().t()  # target image, class
        gxy = t[:, 2:4] * layer.ng  # grid x, y
        gi, gj = gxy.long().t()  # grid x, y indices
        indices.append((b, a, gj, gi)) # collection of array index for b = bounding box, a = anchor box, gj = grount truth y, gi = ground truth x
